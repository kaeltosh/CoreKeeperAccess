...
namespace CoreKeeperAccess.Navigation
{
    ...
    // Magasin des NOMS de balises, persiste PAR MONDE. Le jeu sauvegarde deja les
    // marqueurs eux-memes (position + couleur, entites serialisees) ; nous on ne garde
    // que le nom, indexe par POSITION (la case) - invariant fondateur du reseau de
    // navigation (cf. fiche beacon-navigation). Embryon du futur magasin de noeuds.
    // Magasin des NOMS de balises, persiste PAR MONDE, indexe par POSITION (la case).
    ...
    // DETTE : a sortir dans son propre fichier au prochain build Unity.
    ...
    // annulation). DETTE : a reloger dans Controls/ au prochain build Unity.
